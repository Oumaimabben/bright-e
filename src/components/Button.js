...
const Button = ({ color, colorr, name, onClick, width = "", height = "" }) => {
  return (
    <button
      onClick={onClick}
      className={`relative ${width} ${height} py-1 px-3 border border-black text-black font-semibold bg-white cursor-pointer shadow-${color} hover:transform hover:translate-x-[-2px] hover:translate-y-[2px] hover:shadow-${colorr}`}>
